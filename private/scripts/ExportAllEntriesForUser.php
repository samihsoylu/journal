...
try {
    ...
    $date = new DateTime();
    echo sprintf('[%s] %s%s', $date->format('Y-m-d H:i:s'), $exception, PHP_EOL);
...
final readonly class ExportAllEntriesForUser
{
    ...
    private function zipAllEntries(string $username) : void
    {
        ...
        $command = new Command([
            ...
            '/usr/bin/zip',
            '-r',
            sprintf('%s__%s.zip', $username, $dateString),
            $username . '/',
        ]);
    ...
    private function saveEntryToFile(Entry $entry, string $exportDirectoryPath) : void
    {
        ...
        file_put_contents(
            sprintf('%s/%s/%s.html', $exportDirectoryPath, $category, $title),
            $html,
        );
    }
    ...
    private function writeUserMediaToFile(string $exportDirectoryPath) : void
    {
        ...
        foreach ($images as $imageName) {
            ...
            file_put_contents(sprintf('%s/%s', $mediaDirectory, $imageName), $image->getBinary());
        }
    }
}
